...
/**
 * <code>PackProcessor</code>: A class which processes individual packs in a directory into complete <code>Pack</code> instances.
 */
public final class PackProcessor {
    ...
    /**
     * Compiles all subdirectories in a single root directory into individual unvalidated packs.
     * @param root <code>TracedPath</code>: The root directory in which all packs should be processed.
     * @param isVanilla <code>boolean</code>: Whether or not the packs in the root directory are vanilla packs.
     */
     ...
     * @param isVanilla <code>boolean</code>: Whether or not the path being processed is a vanilla pack.
     */
    public void add(TracedPath path, boolean isVanilla) {
        ...
        try {
            ...
            if (getValidatedPack(identifier) != null || packsUnvalidated.containsKey(identifier)) {
                ...
                throw new DuplicateIdentifierException(newPack.getMetadata().identifierEntry);
            }
    ...
    /**
     * Recursively iterates through the dependencies of a pack and determines if they are satisfied by other, previously validated packs in the program.
     * @return <code>long</code>: The number of dependencies contained in the pack.
     * @param packIdentifier <code>Identifier</code>: The identifier of the pack whose validity must be determined.
     * @throws MissingDependencyException Thrown if a dependency is missing from this <code>PackProcess</code> instance or is otherwise unable to be recognized.
     * @throws InvalidDependencyException Thrown if any dependency of the input pack is invalid.
     */
    private long checkPackDependencyValidity(Identifier packIdentifier) throws MissingDependencyException, InvalidDependencyException {

        // Check whether or not the pack has already been validated - the pack will be in 'packs' or 'packsValidated' if it has been.
        ...
        for (TracedEntry<Metadata> dependencyMetadataEntry : pack.getDependencies().values()) {
            ...
            HashSet<Identifier> validIdentifiers = dependencyMetadata.retrieveCompatibleIdentifiers(Pack.PACKS.keySet());
            validIdentifiers.addAll(dependencyMetadata.retrieveCompatibleIdentifiers(packsValidated.keySet()));

            // If there are any packs that satisfy this dependency, validate this dependency.
            ...
            HashSet<Identifier> compatiblePacks = dependencyMetadata.retrieveCompatibleIdentifiers(packsUnvalidated.keySet());

            // Iterates through all compatible packs and validates them.
            for (Identifier compatiblePackIdentifier : compatiblePacks) {

                // Attempts to recursively check the pack dependency validity of the compatible pack.
                try {

                    // Check the pack dependency validity of the compatible pack, and assign the allowed dependencies accordingly.
                    checkPackDependencyValidity(compatiblePackIdentifier);

                    // If the pack is valid, add it to the validIdentifiers list and set 'validated' to true.
            ...
            // Iterate through the validated identifiers and append their dependencies onto the dependencies of the 'pack' variable.
    ...
    /**
     * Combines the metadata-dependency list key-value pairs from a child map to a parent map.
     * @param child <code>HashMap&lt;String, HashMap&lt;TracedEntry&lt;Metadata&gt;, Collection&lt;Identifier&gt;&gt;&gt;</code>: The child map to assign from.
     * @param parent <code>HashMap&lt;String, HashMap&lt;TracedEntry&lt;Metadata&gt;, Collection&lt;Identifier&gt;&gt;&gt;</code>: The parent map to assign to.
     */
    private void combineMappedMetadatas(HashMap<String, HashMap<TracedEntry<Metadata>, Collection<Identifier>>> child, HashMap<String, HashMap<TracedEntry<Metadata>, Collection<Identifier>>> parent) {

        // Iterates through all identifiers in the child map and assigns its metadata keys to the parent.
        for (String identifier : child.keySet()) {
            ...
            if (!parent.containsKey(identifier)) {
                    ...
                    // If the parent does not already have a map to the identifier string, assign the mapped dependency list to it.
                parent.put(identifier, mappedDependencyList);
            } else {

                // If the parent does already have a map to the identifier tring, assign all key-value pairs to the map.
    ...
    private void filterDependencies(Pack pack) throws InvalidDependencyException {
        ...
        for (String dependencyPackIdentifier : pack.mappedDependencies.keySet()) {
            ...
            // This is the map of all dependencies that reference the dependencyPackIdentifier ID, to the list of pack identifiers which satisfy it.
            HashMap<TracedEntry<Metadata>, Collection<Identifier>> metadatasLists = pack.mappedDependencies.get(dependencyPackIdentifier);

            Iterator<Map.Entry<TracedEntry<Metadata>, Collection<Identifier>>> dependencyIterator = metadatasLists.entrySet().iterator();
            ...
            // Make a copy of the first set of packs so the original collection is not modified.
            ...
            // Iterate through every following dependency in the pack to check for pack overlaps.
            while (dependencyIterator.hasNext()) {
                ...
                // Retain only the packs which overlap between the group collection and this pack list.
    ...
    /**
     * Checks the subdependencies of a pack's dependencies and ensures that none diverge.
     * @param pack <code>Pack</code>: The pack whose dependencies are checked.
     * @param modelPackDependencies <code>Colleciton&lt;TracedEntry&lt;Metadata&gt;&gt;</code>: THe dependencies of the model pack to check against.
     * @param entry <code>TracedEntry&lt;Metadata&gt;</code>: The metadata entry to check.
     * @param entryList <code>Collection&lgt:Identifier&gt;</code>: The list of packs which satisfy the dependency being checked.
     * @throws InvalidDependencyException Thrown if any dependency of the input pack is invalid.
     */
    private void checkSubDependencies(Pack pack, Collection<TracedEntry<Metadata>> modelPackDependencies, TracedEntry<Metadata> entry, Collection<Identifier> entryList) throws InvalidDependencyException {
