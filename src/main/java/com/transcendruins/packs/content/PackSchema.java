...
public final class PackSchema extends Pack {
     ...
     *         this <code>PackSchema</code> instance contains the asset.
     */
        ...
        if (dependenciesEntry.containsValue()) {
            ...
            for (int i : dependenciesJson) {
                ...
                case DependencyType.ASSET -> {
                    ...
                    // same ID.
                    if (dependency.compatible(packId) && !dependency.isLessThan(packId.getVersion())) {

                        throw DependencyException.ambiguousDependency(packId, dependency);
                    }

                    // Check for any matches with asset dependencies.
    ...
    private ImmutableMap<Identifier, AssetSchema> createSchemas(AssetType assetType, TracedPath root) {
        ...
        TracedPath extended = root.extend(assetType.toString());
        HashMap<Identifier, AssetSchema> schemaMap = extended.listRecursiveFiles(TracedPath.JSON).stream().map(path -> {
            ...
            try {
                ...
                .collect(Collectors.toMap( // Map the schemas.
                        ...
                        (parent, child) -> {
                            ...
                            } catch (DuplicateIdentifierException _) {
                            }
