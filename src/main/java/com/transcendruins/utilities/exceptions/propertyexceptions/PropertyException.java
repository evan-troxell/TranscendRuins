...
/**
 * <code>PropertyException</code>: A general exception thrown to indicate an error concerning a JSON field This is a general exception, and should be inherited upon to improve clarity.
 */
public class PropertyException extends LoggedException {
    ...
    public PropertyException(String message, TracedEntry<?> entry, String errorCode) {
        ...
        super(entry.getPathway().path, "JSON parsing exception at field [" + entry.getPathway() + " -> " + entry.getPathway().key + "] : " + message, "Property Exception -> " + errorCode);
    }

    /**
     * Checks whether or not a field is an index or a string, and names it accordingly.
