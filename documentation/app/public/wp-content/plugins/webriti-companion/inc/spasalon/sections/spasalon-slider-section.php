 ...
 */
if ( ! function_exists( 'webriti_companion_spasalon_slider' ) ) :

	function webriti_companion_spasalon_slider() {
		
		$spa_theme_options= webriti_companion_spasalon_slider_data_setup(); 
		...
		if(isset($current_options['slider_post'])) {	
			if(is_numeric ($current_options['slider_post'])) {
				...
				$slider_post = isset($current_options['slider_post']) ? $current_options['slider_post'] : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_banner.jpg';
			}
		}
		else {
		...
		if(isset($current_options['slider_thumbnail_one'])){
			if(is_numeric ($current_options['slider_thumbnail_one'])) {
				...
				$slider_thumbnail_one = isset($current_options['slider_thumbnail_one']) ? $current_options['slider_thumbnail_one'] : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
			}
		}
		else {
		...
		if(isset($current_options['slider_thumbnail_two'])){
			if(is_numeric ($current_options['slider_thumbnail_two'])) {
				...
				$slider_thumbnail_two = isset($current_options['slider_thumbnail_two']) ? $current_options['slider_thumbnail_two'] : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb-2.jpg';
			}
		}
		else{
		...
		if(isset($current_options['slider_thumbnail_three'])){
			if(is_numeric ($current_options['slider_thumbnail_three'])) {
				...
				$slider_thumbnail_three = isset($current_options['slider_thumbnail_three']) ? $current_options['slider_thumbnail_three'] : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb-3.jpg';
			}
		}
		else {
				...
				<section class="slider" id="wrapper">
					<div id="slider">
						<ul class="slide-img slides">
							...
							<?php } 
							if( $current_theme_content['slider_bannerstrip_enable'] == true ) { ?>
								<div class="container topbar-detail">
									<div class="col-md-3">
										<div class="title">
											<?php if($line_one) { ?>
												<h4><?php echo esc_html($line_one); ?></h4> 
											<?php } 
											if($line_two) { ?>
									...
									<div class="col-md-3">
										<div class="addr-detail">
											<address>
												<?php if($call_us_text) { echo esc_html($call_us_text); }  
												if($call_us) { ?> <strong><?php echo esc_html($call_us); ?></strong> <?php } ?>
											</address>
										</div>
									</div>
								</div>
...
if ( function_exists( 'webriti_companion_spasalon_slider' ) ) {
	$section_priority = apply_filters( 'spa_section_priority', 10, 'webriti_companion_spasalon_slider' );
	add_action( 'spasalon_sections', 'webriti_companion_spasalon_slider', absint( $section_priority ) );
}

function webriti_companion_spasalon_slider_data_setup() {	
