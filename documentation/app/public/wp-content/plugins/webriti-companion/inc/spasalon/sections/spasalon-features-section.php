...
if ( ! function_exists( 'webriti_companion_spasalon_features' ) ) :
	function webriti_companion_spasalon_features() {
		...
		$spasalon_service_content  = ! empty($current_options['spasalon_service_content']) ? $current_options['spasalon_service_content'] : webriti_companion_spasalon_get_service_default();
		if(empty($spasalon_service_content)) {
			...
			if(!empty($the_sidebars['sidebar-service'])){
				...
				return apply_filters('spasalon_service_default_content', wp_json_encode($pro_service_data));
			}
			else {		
				return apply_filters('spasalon_service_default_content', wp_json_encode(array(
					array(
		...
		if( $hide_section == true ): ?>
			...
			<section id="section" class="service">
				<div class="container">
					...
					<div class="row">	
						<div class="col-md-12">	
							<div class="section-header">
								<?php if ( ! empty( $spasalon_service_title ) || is_customize_preview() ) {
					...
					<!-- /Section Title -->	
					<?php webriti_companion_spasalon_service_content($spasalon_service_content); ?>
				</div>	
			</section>
...
function webriti_companion_spasalon_service_content( $spasalon_service_content, $is_callback = false ) {
	if ( ! $is_callback ) { ?>
		<?php
	}
	if ( ! empty( $spasalon_service_content ) ) :
									...
									<?php if ( ! empty( $link ) ) : ?>
										<a href="<?php echo esc_url( $link ); ?>" <?php if($opennewtab== "yes"){ echo "target='_blank'";} ?>>
											...
											<figure class="post-thumbnail service-icon">
												<i class="fa <?php echo esc_attr( $icon ); ?> " <?php if ( ! empty( $color ) ) { echo 'style="background-color:' . esc_attr($color) . '"'; } ?>></i>
											</figure>									
											<?php if ( ! empty( $link ) ) : ?>
										</a>
									<?php endif; ?>
 ...
 * @access public
*/
function webriti_companion_spasalon_get_service_default() {
		
	$old_theme_servives = wp_parse_args(  get_option( 'quality_pro_options', array() ), webriti_companion_spasalon_feature_data_setup() );

	if($old_theme_servives['service_one_icon']!= '' || $old_theme_servives['service_one_title']!= '' || $old_theme_servives['service_one_text']!= ''  || $old_theme_servives['service_one_read_more']!= ''	|| $old_theme_servives['service_two_icon']!= '' || $old_theme_servives['service_two_title']!= '' || $old_theme_servives['service_two_text']!= '' || $old_theme_servives['service_two_read_more']!= '' || $old_theme_servives['service_three_icon']!= '' || $old_theme_servives['service_three_title']!= '' || $old_theme_servives['service_three_text']!= '' || $old_theme_servives['service_three_read_more']!= '' || $old_theme_servives['service_four_icon']!= '' || $old_theme_servives['service_four_title']!= '' || $old_theme_servives['service_four_text']!= '' || $old_theme_servives['service_four_read_more']!= '') {
...
if ( function_exists( 'webriti_companion_spasalon_features' ) ) {
	$section_priority = apply_filters( 'spasalon_section_priority', 10, 'webriti_companion_spasalon_features' );
	add_action( 'spasalon_sections', 'webriti_companion_spasalon_features', absint( $section_priority ) );	
}

function webriti_companion_spasalon_feature_data_setup() {	
