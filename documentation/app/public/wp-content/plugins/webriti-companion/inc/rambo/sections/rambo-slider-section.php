 ...
 */
if ( ! function_exists( 'webriti_companion_rambo_slider' ) ) :
	function webriti_companion_rambo_slider() {
		...
		if(isset($current_options['slider_post'])) {	
			if(is_numeric ($current_options['slider_post']))
			{
				...
				$slider_post = isset($current_options['slider_post']) ? $current_options['slider_post'] : WC__PLUGIN_URL.'/inc/rambo/img/slider.jpg';
			}
		}
		else {
			...
			<div class="main_slider">
				<div class="carousel-inner">
					...
					<div id="post-<?php the_ID(); ?>" class="item active">
						<div class="item" style="background-image:url(<?php echo esc_url($slider_post); ?>); width: 100%; height: 60vh; background-position: center center; background-size: cover; z-index: 0;" >
							<div class="container slider_con">
								...
								<?php if($current_options['slider_readmore_text']!=''){ ?>
									<a class="slide-btn" href="<?php echo esc_url($current_options['readmore_text_link']); ?>" <?php if($current_options['readmore_target'] == true) { echo "target='_blank'"; } ?> class="flex-btn"><?php echo esc_html($current_options['slider_readmore_text']); ?></a>
								<?php } ?>
							</div>
						</div>
					</div>
...
if ( function_exists( 'webriti_companion_rambo_slider' ) ) {
	$section_priority = apply_filters( 'honeypress_section_priority', 11, 'webriti_companion_rambo_slider' );
	add_action( 'rambo_business_template_sections', 'webriti_companion_rambo_slider', absint( $section_priority ) );
}