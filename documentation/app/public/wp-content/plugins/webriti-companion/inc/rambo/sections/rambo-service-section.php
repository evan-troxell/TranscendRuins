 ...
 */
if ( ! function_exists( 'webriti_companion_rambo_services' ) ) :
	function webriti_companion_rambo_services() {
		...
		if($current_options['home_service_enabled']==false) {
			$rambo_service_content  = ! empty($current_options['rambo_service_content']) ? $current_options['rambo_service_content'] : webriti_companion_rambo_get_service_default();
			...
			if(empty($rambo_service_content)) {
				...
				if (!empty($current_options['slider_post'])){
					...
					if(!empty($the_sidebars['sidebar-service'])){
						...
						$rambo_service_content = wp_json_encode($pro_service_data);	
					}
					else {								
			...
			function webriti_companion_rambo_service_content( $rambo_service_content, $is_callback = false ) {
				if ( ! $is_callback ) { ?>
						...
						if ( ! empty( $rambo_service_content ) ) {
							...
							for($i=0; $i<=3; $i++) { ?>	
								<div id="rambo_feature_page_widget-2" class="span3 home_service widget widget_rambo_feature_page_widget">
									<span class="fa-stack fa-4x icon_align_center"><a href="#" target="_blank"><i class="fa <?php echo esc_attr($icon[$i]); ?> home_media_icon_1x fa-inverse"></i></a></span>
									<a href="#" target="_blank">
										<h2 class="widget-title"><?php echo esc_html($title[$i]); ?></h2>
									</a>
			...
			<div class="home_service_section">
				<div class="container">
					...
					<!-- /Home Service Section -->
				 	<?php webriti_companion_rambo_service_content( $rambo_service_content ); ?>
					<!-- /Home Service Section -->	
				</div>
			</div>
		<?php }
...
function webriti_companion_rambo_get_service_default() {
	
	$old_theme_servives = wp_parse_args(  get_option( 'quality_pro_options', array() ), webriti_companion_rambo_service_data_setup() );
...
if ( function_exists( 'webriti_companion_rambo_services' ) ) {
	$section_priority = apply_filters( 'honeypress_section_priority', 11, 'webriti_companion_rambo_services' );
	add_action( 'rambo_business_template_sections', 'webriti_companion_rambo_services', absint( $section_priority ) );
}

function webriti_companion_rambo_service_data_setup()
{	
