 ...
 */
if ( ! function_exists( 'webriti_companion_rambo_child_services' ) ) :
	function webriti_companion_rambo_child_services() {
		$current_options = wp_parse_args(  get_option( 'rambo_pro_theme_options', array() ), rambo_theme_data_setup() );
			if($current_options['home_service_enabled']==false) {
				...
				$rambo_child_service_content  = ! empty($current_options['rambo_service_content']) ? $current_options['rambo_service_content'] : webriti_companion_rambo_child_get_service_default();
				...
				if(empty($rambo_child_service_content)) {
					...
					if (!empty($current_options['slider_post'])) {
						...
						if(!empty($the_sidebars['sidebar-service'])) {
							...
							$rambo_child_service_content = wp_json_encode($pro_service_data);							
						}
						else {			
							$rambo_service_content = wp_json_encode( array(
								array(
				...
				function webriti_companion_rambo_child_service_content( $rambo_child_service_content, $is_callback = false ) {
					if ( ! $is_callback ) { ?>
					<div class="row-fluid">
					<?php } 
						if ( ! empty( $rambo_child_service_content ) ) {
										...
										<div class="service-area">
											<?php if ( ! empty( $title ) ) : ?>				
												<h2 class="widget-title">
													<a <?php if(!empty($link)){?> href="<?php echo esc_url( $link ); ?>" <?php } ?><?php if($opennewtab == 'yes'){ echo 'target="_blank"';}?>><?php echo esc_html( $title ); ?>
													</a>
												</h2>
											<?php endif;
											if ( ! empty( $text ) ) : ?>
				...
				<div class="<?php echo esc_attr($service_class); ?>">
					<div class="container">
						...
						<!-- /Home Service Section -->
					 	<?php webriti_companion_rambo_child_service_content( $rambo_child_service_content ); ?>
						<!-- /Home Service Section -->	
					</div>
				</div>
		<?php }
...
function webriti_companion_rambo_child_get_service_default() {
	
	$old_theme_servives = wp_parse_args(  get_option( 'quality_pro_options', array() ), webriti_companion_rambo_child_plugin_data_setup() );
	...
	if($old_theme_servives['service_one_icon']!= '' || $old_theme_servives['service_one_title']!= '' || $old_theme_servives['service_one_text']!= ''  || $old_theme_servives['service_one_read_more']!= '' || $old_theme_servives['service_two_icon']!= '' || $old_theme_servives['service_two_title']!= '' || $old_theme_servives['service_two_text']!= '' || $old_theme_servives['service_two_read_more']!= '' || $old_theme_servives['service_three_icon']!= '' || $old_theme_servives['service_three_title']!= '' || $old_theme_servives['service_three_text']!= '' || $old_theme_servives['service_three_read_more']!= '' || $old_theme_servives['service_four_icon']!= '' || $old_theme_servives['service_four_title']!= '' || $old_theme_servives['service_four_text']!= '' || $old_theme_servives['service_four_read_more']!= '') {
 		...
 		return apply_filters('rambo_child_service_default_content', wp_json_encode(array(
			array(
...
if ( function_exists( 'webriti_companion_rambo_child_services' ) ) {
	$section_priority = apply_filters( 'honeypress_section_priority', 11, 'webriti_companion_rambo_child_services' );
	add_action( 'rambo_business_template_sections', 'webriti_companion_rambo_child_services', absint( $section_priority ) );
}

function webriti_companion_rambo_child_plugin_data_setup() {				
