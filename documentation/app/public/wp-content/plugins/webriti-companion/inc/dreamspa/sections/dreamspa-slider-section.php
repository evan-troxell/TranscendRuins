	...
	function webriti_companion_spasalon_slider() {
		...
		if(isset($current_options['dreamspa_slider_post'])){	
			if(is_numeric($current_options['dreamspa_slider_post']))
			{
				...
				$dreamspa_slider_post = isset($current_options['dreamspa_slider_post']) ? $current_options['dreamspa_slider_post'] : WC__PLUGIN_URL . '/inc/spasalon/images/slider/home_banner.jpg';
			}
		}
		else {
		...
		if(isset($current_options['slider_title'])){	
			...
			{
				$slider_title = isset($current_options['dreamspa_slider_post'])?   get_the_title($current_options['slider_title']) : __('The Essence of Natural Beauty','webriti-companion');
			}
			else{
		...
		if(isset($current_options['slider_desc'])){	
			...
			{
				$slider_desc = isset($current_options['dreamspa_slider_post'])?   get_the_content($current_options['slider_desc']) : __('We at Dream Spa provide services to the natural of the clients !','webriti-companion');
			}
			else{
				$slider_desc = $current_options['slider_desc'];
			}
		}
		else {
					...
					<div id="slider">
						<ul class="slide-img slides">
							<?php if($dreamspa_slider_post) { ?>
								<img alt="slide1" src="<?php echo esc_url($dreamspa_slider_post); ?>"/>	
							<?php } 
							} ?>
							<div class="container caption-overlay text-<?php echo esc_attr($current_options['slider_caption_align']); ?>">
								<?php if($slider_title) { ?>
									<h1 class="slider_txt"><?php  echo esc_html($slider_title); ?></h1>
								<?php } 
								if($slider_desc) { ?>
									<div class="txt"><p><?php echo esc_html($slider_desc); ?>
									<?php } ?>	
								</div>
							</div>
						</ul>
...
function webriti_companion_slider_theme_data_setup()
{	
