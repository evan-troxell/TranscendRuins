 ...
 */
if (!function_exists('webriti_companion_appointment_slider')) :
    function webriti_companion_appointment_slider() {
        ...
        if ($slider_setting['home_banner_enabled'] == 0 || $slider_setting['home_banner_enabled'] == '') {
            ...
            if( class_exists('Spice_Slider_Pro') && !empty(get_theme_mod('wc_sps_shortcode')) && (get_theme_mod('slide_variation','banner_image')=='post_slider')){
                ...
                <div class="homepage-mycarousel <?php echo esc_attr($slider_setting['slider_radio']); echo (isset($slider_class)) ? esc_attr($slider_class) : '';  ?>">
                                ...
                                <ol class="carousel-indicators" role="tablist" tabindex="0">
                                    ...
                                    if ($the_query->have_posts()) {
                                        while ($the_query->have_posts()) {
                                            $the_query->the_post(); ?>
                                            <li data-target="#carousel-example-generic" data-slide-to="<?php echo absint($i); ?>" class="<?php if ($i == 0) { echo 'active'; } ?>"></li>
                                            <?php
                                            $i++;
                                        }
                                    } ?>
                                            ...
                                            if (!has_post_thumbnail()) { ?>
                                                ...
                                                <div class="container slide-caption">
                                                    ...
                                                    if ($the_query->post_content == "") {
                                                        echo wp_kses_post(appointment_get_home_slider_excerpt());
                                                    } ?>
                                                </div>
                                            <?php } else { ?>
                                                <div class="container slide-caption">
...
if (function_exists('webriti_companion_appointment_slider')) {
    $section_priority = apply_filters('appointment_section_priority', 10, 'webriti_companion_appointment_slider');
    add_action('appointment_sections', 'webriti_companion_appointment_slider', absint($section_priority));
}