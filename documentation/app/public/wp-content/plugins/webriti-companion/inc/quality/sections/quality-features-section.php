 ...
 */
if (!function_exists('webriti_companion_quality_features')) :

    function webriti_companion_quality_features() {
        ...
        $quality_service_content = !empty($current_options['quality_service_content']) ? $current_options['quality_service_content'] : webriti_companion_quality_get_service_default();
        ...
        function webriti_companion_quality_service_section_html_class($type) {
            
            $theme = wp_get_theme();
            if($theme->name =='Quality orange'){
                $service_child_setting = wp_parse_args(get_option('quality_pro_options', array()), webriti_companion_quality_orange_default_data());
            }
            elseif($theme->name =='Quality green'){
                $service_child_setting = wp_parse_args(get_option('quality_pro_options', array()), webriti_companion_quality_green_default_data());
            }
            elseif($theme->name =='Quality blue'){
                $service_child_setting = wp_parse_args(get_option('quality_pro_options', array()), webriti_companion_quality_blue_default_data());
            }
            elseif($theme->name =='Heroic'){
                $service_child_setting = wp_parse_args(get_option('quality_pro_options', array()), webriti_companion_heroic_default_data());
            }
            else{
                $service_child_setting = wp_parse_args(get_option('quality_pro_options', array()), webriti_companion_quality_theme_data_setup());
            }

            $themeName = $theme->name;
            if ($type == 'sectionClass') {
                $serviceSectionClass = '';
                if ($themeName == 'Quality') { $serviceSectionClass = '';  } 
                ...
                echo esc_attr($serviceSectionClass);
            } 
            elseif ($type == 'articleClass') {
                if ($themeName == 'Quality' ||($themeName == 'Quality blue') || $themeName == 'Quality green' || $themeName == 'Quality orange' && $service_child_setting['service_rotate_layout_setting']=='default' || $themeName == 'Heroic' && $service_child_setting['service_slide_layout_setting']=='default') {
                    ...
                    echo esc_attr($serviceArticleClass='text-center');
                }
            }
        }
        add_filter('serviceSection', 'webriti_companion_quality_service_section_html_class', '', 1);

        // Add div wrapper for specific child theme
        function webriti_companion_quality_div_service_wrapper_fn($type, $class) {
            $theme = wp_get_theme();
            $themeName = $theme->name;
            $openDiv = '';
            ...
            echo wp_kses_post($openDiv);
        }
        add_filter('quality_div_service_wrapper', 'webriti_companion_quality_div_service_wrapper_fn', '', 2);
        ...
        if ($hide_section == true) { ?>
            <section id="section-block" class="service <?php apply_filters('serviceSection', 'sectionClass'); ?>">
                <div class="container">
                        ...
                        <div class="row">
                            <div class="section-header">
                                ...
                                if (!empty($quality_service_title) || is_customize_preview()) {
                                    echo '<p>' . esc_html($quality_service_title) . '</p>';
                                }
                                if (!empty($quality_service_subtitle) || is_customize_preview()) {
                                    echo '<h1 class="widget-title">' . wp_kses_post($quality_service_subtitle) . '</h1>';
                                }
                    ...
                    <?php }
                    webriti_companion_quality_service_content($quality_service_content); ?>
                </div>
            </section>
        <?php }
    }
...
function webriti_companion_quality_service_content($quality_service_content, $is_callback = false) {
    if (!$is_callback) {
        ?>
                                ...
                                <article class="post <?php apply_filters('serviceSection', 'articleClass'); ?>">
                                    ...
                                    <?php } 
                                        ...
                                        <?php if (!empty($icon)) : ?>
                                            <figure class="post-thumbnail">
                                                <?php if (!empty($link)) : ?>
                                                    <a href="<?php echo esc_url($link); ?>" <?php if ($open_new_tab == 'yes' || $open_new_tab == 'on') { echo "target='_blank'"; } ?>><i class="fa <?php echo esc_attr($icon); ?>"></i></a>
                                                    <?php endif; ?>
                                                    <?php if (empty($link)) : ?>
                                    ...
                                    if (!empty($title)) :
                                        ...
                                        if (empty($link)) { ?>
                                            ...
                                            <div class="entry-header">
                                                <h3 class="entry-title"><a href="<?php echo esc_url($link); ?>" <?php if ($open_new_tab != 'no') { ?>target="_blank" <?php } ?> ><?php echo esc_html($title); ?></a></h3></div>
                                        <?php }
                                        ?>
                                    <?php endif; ?>
                                    <?php if (!empty($text)) : ?>
 ...
 * @access public
 */
function webriti_companion_quality_get_service_default() {

    $old_theme_servives = wp_parse_args(get_option('quality_pro_options', array()), webriti_companion_quality_feature_data_setup());
    ...
    if ($old_theme_servives['service_one_icon'] != '' || $old_theme_servives['service_one_title'] != '' || $old_theme_servives['service_one_text'] != '' || $old_theme_servives['service_two_icon'] != '' || $old_theme_servives['service_two_title'] != '' || $old_theme_servives['service_two_text'] != '' || $old_theme_servives['service_three_icon'] != '' || $old_theme_servives['service_three_title'] != '' || $old_theme_servives['service_three_text'] != '' || $old_theme_servives['service_four_icon'] != '' || $old_theme_servives['service_four_title'] != '' || $old_theme_servives['service_four_text'] != '' || $old_theme_servives['service_five_icon'] != '' || $old_theme_servives['service_five_title'] != '' || $old_theme_servives['service_five_text'] != '' || $old_theme_servives['service_six_icon'] != '' || $old_theme_servives['service_six_title'] != '' || $old_theme_servives['service_six_text'] != '') {
        ...
        return apply_filters('quality_service_default_content', wp_json_encode(array(
            array(
...
if (function_exists('webriti_companion_quality_features')) {
    $section_priority = apply_filters('quality_section_priority', 10, 'webriti_companion_quality_features');
    add_action('quality_sections', 'webriti_companion_quality_features', absint($section_priority));
}

function webriti_companion_quality_feature_data_setup() {
    return $theme_options = array(
        ...
        'service_five_text' => esc_html__('Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed dosit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut.', 'webriti-companion'),
        'service_six_text' => esc_html__('Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed dosit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut.', 'webriti-companion'),
    );
}