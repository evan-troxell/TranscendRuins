			...
			if ( ( count( $json ) == 1 && '' === $json[0] ) || empty( $json ) ) {
				if ( ! empty( $this_default ) ) {
					...
					<input type="hidden"
					       ...
					       value="<?php echo esc_attr( wp_json_encode( $this_default ) ); ?>"/>
					<?php
				} else {
					$this->iterate_array(); ?>
					<input type="hidden"
	...
	private function icon_picker_control($value = '', $show = ''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
		    ...
		    <span class="description customize-control-description">
                <?php
                /* translators: %1$s: URL of the page and %s: redirect on the another page */
                echo sprintf( esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'webriti-companion' ), sprintf( '<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html('http://fontawesome.io/icons/'))
                ); ?>
            </span>
