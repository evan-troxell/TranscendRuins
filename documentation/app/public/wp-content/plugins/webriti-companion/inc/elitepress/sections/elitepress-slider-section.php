...
if (!function_exists('webriti_companion_elitepress_slider')) :
	function webriti_companion_elitepress_slider() {
		...
		if(empty($slide_options))
		{
			...
			{	
				...
				if( $slider->have_posts() ) {
					...
					endwhile; 
					$slide_options = wp_json_encode($pro_slider_data_old);
				}		
			}
		}
		...
		wp_register_script('elitepress-slider', get_template_directory_uri() . '/js/front-page/slider.js', array('jquery'), '1.0', true);
							...
							<li style="background-image:url(<?php echo esc_url($slide_iteam->image_url); ?>);width: 100%; height: 60vh; background-position: center center; background-size: cover; z-index: 0;">
								...
								<div class="container flex-slider-center">
									...
									<?php if($img_description !=''){?>
										<div class="slide-text-bg2"><h3><?php echo esc_html($img_description); ?></h3></div>
									<?php } ?>
...
if (function_exists('webriti_companion_elitepress_slider')) {
    $section_priority = apply_filters('elitepresss_section_priority', 11, 'webriti_companion_elitepress_slider');
    add_action('elitepress_sections', 'webriti_companion_elitepress_slider', absint($section_priority));
}

function webriti_companion_elitepress_flexslider_scripts()
{	
	wp_enqueue_style('elitepress-flexslider', WC__PLUGIN_URL . '/inc/controls/flexslider/css/flexslider/flexslider.css', array(), '2.2.0');
	wp_enqueue_script('elitepress-jquery-flexslider', WC__PLUGIN_URL .'/inc/controls/flexslider/js/flexslider/jquery.flexslider.js', array('jquery'), '2.2.2', true);	
	wp_enqueue_script('jquery-flex-element', WC__PLUGIN_URL .'/inc/controls/flexslider/js/flexslider/flexslider-element.js', array('jquery'), '2.2.2', true);		
}
add_action('wp_enqueue_scripts', 'webriti_companion_elitepress_flexslider_scripts');