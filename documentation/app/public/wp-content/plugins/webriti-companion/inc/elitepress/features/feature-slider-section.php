...
function webriti_companion_elitepress_slider_customizer( $wp_customize ) {

	// list control categories	
	if ( ! class_exists( 'WP_Customize_Control' ) ) return NULL;
	class Webriti_Companion_Category_Dropdown_Custom_Control extends WP_Customize_Control
	{
		private $cats = false;
	...
	//Hide slider
	$wp_customize->add_setting('elitepress_lite_options[home_banner_enabled]', array(
        'default' => true,
		'capability'     => 'edit_theme_options',
	...
	$wp_customize->add_setting('elitepress_lite_options[animationSpeed]', array(
        'default' => '1500',
		'type' => 'option',
	...
	//Slide Show Speed
	$wp_customize->add_setting('elitepress_lite_options[slideshowSpeed]', array(
        'default' => '2500',
		'type' => 'option',
		'sanitize_callback' => 'elitepress_sanitize_select',
    ));
...
add_action( 'customize_register', 'webriti_companion_elitepress_slider_customizer' );

function webriti_companion_elitepress_slider_sanitize_layout( $value ) {
	if ( ! in_array( $value, array( 'Uncategorized','category_slider' )))    
	return $value;
}

// Elitepress default slider data.
if ( ! function_exists( 'webriti_companion_elitpress_slider_default_customize' ) ) :
	
	function webriti_companion_elitpress_slider_default_customize( $wp_customize ) {	
		if(get_option('elitepress_lite_options')!='')
		{
			...
			if ( ! empty( $elitepress_slider_content_control ) ) {
				...
				if( $slider->have_posts()) {	
					$elitepress_slider_content_control->default = wp_json_encode( array() );
					...
					endwhile; 
					$elitepress_slider_content_control->default = wp_json_encode($pro_slider_data);
				}
			}
		} 
		else {
	...
	add_action( 'customize_register', 'webriti_companion_elitpress_slider_default_customize' );
endif;