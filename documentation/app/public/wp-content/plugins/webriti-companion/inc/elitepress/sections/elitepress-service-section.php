...
function webriti_companion_elitepress_service_content( $elitepress_service_content, $is_callback = false ) {
	$theme = wp_get_theme(); // gets the current theme
	if ( ! $is_callback ) { ?> <div class="row"> <?php }
		if ( ! empty( $elitepress_service_content ) ) {
			...
			$title = array (__('Ipsum pulvinar','webriti-companion'), __('Lorem ipsum','webriti-companion'), __('Integer ultricies','webriti-companion'), __('Integer condimentum','webriti-companion'));
                    ...
                    <div class="media service-area">
                        ...
                        <div class="media-body">
                            <h4 class="entry-title"><?php echo esc_html($title[$i]); ?></h4>
               			 	<p><?php echo esc_html__('Morbi leo risus, porta ac consectetur ac, vestibulum at eros. Cras mattis consectetur purus sit amet ferment etiam porta sem malesuada magna mollis.','webriti-companion'); ?></p>
                            <div class="service-btn">
                            	<a title="<?php esc_attr_e('Read More','webriti-companion'); ?>" href="#"><?php esc_html_e('Sed gravida', 'webriti-companion'); ?></a>
                            </div>
                		</div>
                    </div>
                </div>
	...
	if ( ! $is_callback ) { ?> </div> <?php }
}

if (!function_exists('webriti_companion_elitepress_services')) :
	function webriti_companion_elitepress_services() {
		...
		if(!empty($current_options)){
			if(empty($elitepress_service_content)) {
                ...
                if($old_theme_servives['service_one_icon']!= '' || $old_theme_servives['service_one_title']!= '' || $old_theme_servives['service_one_description']!= '' || $old_theme_servives['service_two_icon']!= '' || $old_theme_servives['service_two_title']!= '' || $old_theme_servives['service_two_description']!= '' || $old_theme_servives['service_three_icon']!= '' || $old_theme_servives['service_three_title']!= '' || $old_theme_servives['service_three_description']!= '' || $old_theme_servives['service_four_four']!= '' || $old_theme_servives['service_four_title']!= '' || $old_theme_servives['service_four_description']!= '') {

                	$elitepress_service_content = wp_json_encode( array(
                        array(
		...
		if( $current_options['service_section_enabled']== true ) { ?>
			...
			<section class="<?php echo esc_attr($service_class);?>">
				<div class="container">		
					...
					<!-- /Section Title -->
					<?php webriti_companion_elitepress_service_content( $elitepress_service_content ); ?>
				</div>
			</section>
...
if ( function_exists( 'webriti_companion_elitepress_services' ) ) {
	$section_priority = apply_filters( 'elitepress_section_priority', 13, 'webriti_companion_elitepress_services' );
	add_action( 'elitepress_sections', 'webriti_companion_elitepress_services', absint( $section_priority ) );
}